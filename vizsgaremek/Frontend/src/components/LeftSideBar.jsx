...
function LeftSideBar() {
  ...
  return (
    <div>
      <div className="sm:first:col-span-2 py-14 px-11 rounded-lg max-w-lg" style={{ backgroundColor: '#FFE8D2', marginLeft: '35px' }}>
        ...
        <ul className="mt-6 sm:mt-10">
          {categories.map((category, index) => (
            <li key={index} className="pt-2 pb-4 mb-2 last:mb-0 border-b border-black border-solid">
              <Link to={`/kategoria/${category.categoryID}`} className="flex items-center justify-between text-black hover:text-black text-lg sm:text-xl font-medium" onClick={() => handleCategoryClick(category.categoryID)}> 
