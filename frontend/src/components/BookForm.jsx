...
function BookForm() {
    ...
    const kuldes = async () => {
        try {
            ...
            if (response.ok) {
                ...
                navigate("/"); // Navigálás sikeres beküldés után
            } else {
                throw new Error('Valami hiba van szerver oldalon!');
            }
        } catch (error) {
    ...
    const handleSelectChange = (event) => {
        const value = event.target.value ? JSON.parse(event.target.value) : { categoryID: '', category: '' };
        setSelectedCategory(value);  // Ez frissíti a kiválasztott kategória állapotát
        setFormData(prevState => ({
            ...prevState,
            categoryID: value.categoryID  // Ez frissíti a form adatokat a kiválasztott kategória ID-jével
        }));
    };
    ...
    return (
        <div className="flex items-center justify-center p-12">
            <div className="mx-auto w-full max-w-4xl bg-white p-8">
                <h2 className="text-2xl font-bold text-center mb-10">Új könyv felvitele</h2>
                <form onSubmit={onSubmit} className="grid grid-cols-2 gap-6">
                    ...
                    <div className="mb-2 col-span-2">
                        <label className="block mb-2 text-base font-semibold text-[#07074D]">Kategória</label>
                        <select
                            id="category-select"
                            className="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
                            onChange={handleSelectChange}
                            value={JSON.stringify(selectedCategory)} // A kiválasztott kategória teljes objektumát használjuk itt
                            required
                        >
