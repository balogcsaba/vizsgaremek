...
function BookFormModify() {
    ...
    const [selectedCategory, setSelectedCategory] = useState({ categoryID: '', category: '' });
    ...
    useEffect(() => {
        if (BookID) {
            // fetch(`http://localhost:3001/books/${BookID}`) ez a jó, csak kivettem //
            fetch(`http://localhost:3001/books/${BookID}`)
                .then(res => res.json())
                .then(data => {
                    //setFormData(data); ez a jó csak kivettem//
                    setFormData(data);
                    const foundCategory = categories.find(cat => cat.categoryID === data.categoryID);
                    if (foundCategory) {
                        setSelectedCategory(foundCategory);
                    }
                })
                .catch(err => console.error("Nem tudtam betölteni az adatokat:", err));
        }
    }, [BookID]);
    ...
    const onSubmit = async (event) => {
        ...
        try {
            ...
            if (response.ok) {
                navigate("/"); // elnavigájuk a főoldalra
            } else {
                throw new Error('Szerver oldalon van valami hiba');
            }
        } catch (error) {
