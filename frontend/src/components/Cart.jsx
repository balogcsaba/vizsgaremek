...
function Cart() {
  ...
  const handleCheckout = () => {
    ...
    const borowsbooks = carts.map(item => ({ 
      BookID: item.BookID, 
      userID: "localStorage-ből vett userID",
      cartID: cartID,
      title: item.title,
      quantity: item.quantity, 
      price: item.price, 
      /* total_price: (item.price * item.quantity).toFixed(2) csak akkor, ha kimomentelt rész vissazkerülne*/
      start_of_borrowment: new Date(),
      end_of_borrowment: new Date(new Date().getTime() + 7 * 24 * 60 * 60 * 1000) })); // Könyvek tömb létrehozása és feltöltése
  ...
  if(carts.length === 0){
    return (
      <div className="container mx-auto mt-10">
        <h1 className=' h-[55vh] flex justify-center items-center text-4xl'>Cart is Empty</h1>
      </div>
    );
  }
              ...
              <div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5" key={cart.BookID}>
                <div className="flex w-2/5">
                  ...
                  <div className="flex flex-col justify-between ml-4 flex-grow">
                    ...
                    <div className="font-semibold hover:text-red-500 text-gray-500 text-xs cursor-pointer" onClick={() => removeProduct(cart.BookID)}>Remove</div>
                  </div>
                </div>
